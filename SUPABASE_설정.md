...
함께 생성되는 보안 함수는 다음 기능에 사용됩니다.

- `find_member_email`: 이름과 연락처가 일치하는 회원의 이메일을 일부 가려서 표시
- `delete_current_user`: 로그인한 회원 본인의 계정과 프로필 사진 삭제

기존에 `schema.sql`을 실행한 프로젝트도 이번 기능 적용을 위해 파일 전체를 다시 실행해야 합니다. 모든 구문은 재실행할 수 있도록 작성되어 있습니다.
...
6. 로그아웃 후 아이디 찾기와 비밀번호 재설정 메일 확인
7. 테스트 회원으로 다시 로그인한 뒤 마이페이지 회원 탈퇴 확인
8. 일반 문의 제출 후 `inquiries` 테이블 확인
9. 강사 지원 제출 후 `instructor_applications` 테이블 확인
